...
DOWNLOAD_DIR: str = f'/data' #'/media/ponbac/BigHDD/ethereum'# 
COLUMNS_TO_SAVE: Optional[List[str]] = ['block_number', 'transaction_index', 'trace_address', 'from_address',
                                        'to_address', 'value', 'status']  # 'None' = Download all columns.
DTYPES = {'block_number': int, 'transaction_index': pd.Int32Dtype(), 'from_address': str,
          'to_address': str, 'value': float, 'trace_address': str, 'status': int}
...
N_BLOBS: int = 6000 #12000 #18000 #23478
while END_OFFSET < N_BLOBS + STEP_SIZE:
    print('[DOWNLOADING]')
    storage: EthereumStorage = GoogleCloudStorage()
    storage.download(BUCKET_NAME, f'{BQ_TABLE_NAME}/',
                     f'{DOWNLOAD_DIR}/{BQ_TABLE_NAME}', MAX_BLOBS, use_cols=COLUMNS_TO_SAVE, dtypes=DTYPES, start_offset=START_OFFSET, end_offset=END_OFFSET)
    ...
    print('\n[UPLOADING]')
    if subprocess.call(['gsutil', '-m', 'cp', '-r', f'{DOWNLOAD_DIR}/{BQ_TABLE_NAME}', f'gs://{BUCKET_NAME}/pruned/']) == 0:
...
subprocess.call(['gsutil', '-m', 'cp', '-r',
                    f'gs://{BUCKET_NAME}/pruned/', DOWNLOAD_DIR])
...
print('\n[UPLOADING]')
if subprocess.call(['gsutil', '-m', 'cp', '-r', f'{DOWNLOAD_DIR}/pruned/traces/traces-sorted.csv', f'gs://{BUCKET_NAME}/sorted/']) == 0:
