...
class Poison(BaseBlacklist):
    ...
    def make_blacklist(
        ...
        blacklist = pd.Series(
            [True] * len(self.initial_blacklist), self.initial_blacklist
        ).to_dict()
        ...
        for tx_chunk in self.data_loader.yield_traces(
            chunk_size, start_block, end_block
        ):
            pd.options.mode.chained_assignment = None
            nonzero_traces = tx_chunk[
                (tx_chunk.value > "0") & (tx_chunk.status == 1)
            ].dropna(subset=["to_address", "value"])
            ...
            if chunk_counter % 1000 == 0 or chunk_counter == 1:
                ...
                if chunk_counter == 1:
                    run_data.to_csv(f"{self.save_dir}/poison-rundata.csv", index=False)
                else:
                    run_data.to_csv(
                        f"{self.save_dir}/poison-rundata.csv",
                        mode="a",
                        header=False,
                        index=False,
                    )
        ...
        dict_to_csv(
            blacklist, ["address", "flagged"], f"{self.save_dir}/poison-result.csv"
        )
